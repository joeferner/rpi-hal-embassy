...
// one for the LAN9514's bulk traffic. The controller arbitrates between
// them in hardware, so neither side has to know the other exists, and
// running out of channels is an error rather than a silent queue.
//
// The two halves are also written in different styles, deliberately, and
// this example is where that gets tested:
//
// - The **keyboard** goes through `embassy-usb-host`, whose transfers are
//   interrupt-driven and await.
// - The **Ethernet** goes through rpi-hal's own `Lan9514Driver`, whose
//   transfers are the blocking ones — it busy-waits on `HCINT` for the
//   microseconds a bulk transfer takes.
//
// `usb::dwc2::on_irq` only services channels an async transfer is waiting
// on, and leaves a channel being polled by the blocking API entirely
// alone, which is what lets the two share one controller and one
// interrupt handler. The cost is that a blocking transfer holds the
// executor for its duration, so a stalled one would add latency to the
// keyboard — bounded, since the blocking path times out at 50ms.
//
...
/// How often the application nudges the LAN9514 driver to look for a
/// frame.
///
/// Nothing interrupt-driven can replace this: a bulk IN with no frame
/// waiting simply NAKs, so there is no event meaning "a frame arrived",
/// only an answer to a question the host keeps asking. The interval is
/// the latency-versus-wake-ups trade, and it belongs to the application.
...
async fn start_ethernet(
    ...
    let Some(mut channel) = host.alloc_channel() else {
    ...
    let driver = Lan9514Driver::new(lan9514, channel, timer, kit.mac);
    ...
    if let (Ok(net), Ok(poll), Ok(echo)) =
        (net_task(runner), poll_task(), echo_task(stack, console))
    {
        spawner.spawn(net);
        spawner.spawn(poll);
        spawner.spawn(echo);
