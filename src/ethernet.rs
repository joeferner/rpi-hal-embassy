//! `embassy-net` adapter over `rpi-hal`'s USB-Ethernet drivers: a queue
//! pair for the stack
//! ([`EthernetDriver`](crate::ethernet::EthernetDriver)) and a task that
//! moves frames between those queues and the chip's bulk endpoints
//! ([`EthernetRunner`](crate::ethernet::EthernetRunner)). Build both with
//! [`new`](crate::ethernet::new).
//!
//! Generic over
//! [`EthernetAsync`](rpi_hal::usb::ethernet::EthernetAsync), so the same
//! adapter serves a Pi 2B/3B's LAN9514 and a 3B+'s LAN7800 — which are
//! different chips with nothing in common below that trait. A board that
//! names its chip can keep doing so through
//! [`crate::lan9514`], which is this module under the older spelling.
//!
...
//!
//! [`new`](crate::ethernet::new) takes two `Channel`s, and the runner
...
/// [`Ethernet::MTU`](rpi_hal::usb::ethernet::Ethernet::MTU),
/// because `ch::State` takes it as a const generic and `E::MTU` is not
/// usable in that position on stable Rust. Every driver this adapter
/// serves declares the same 1514, and [`new`] asserts it — so the
/// constraint is checked rather than assumed, and a future chip with a
/// different frame size fails loudly here instead of quietly truncating.
///
/// It is also what makes a board able to swap one interface for another
/// under a live stack: `ch::Device<'_, MTU>` differs by this parameter, so
/// a shared value is what keeps the two interchangeable.
pub const MTU: usize = 1514;

/// The packet queues [`new`] builds a
/// [`EthernetDriver`] and [`EthernetRunner`] out of, with room for `N_RX`
...
/// [`MTU`] each.
pub struct EthernetState<const N_RX: usize, const N_TX: usize> {
    inner: ch::State<MTU, N_RX, N_TX>,
}

impl<const N_RX: usize, const N_TX: usize> EthernetState<N_RX, N_TX> {
...
impl<const N_RX: usize, const N_TX: usize> Default for EthernetState<N_RX, N_TX> {
    /// The same as [`EthernetState::new`].
    fn default() -> Self {
        Self::new()
    }
}
...
/// it hands the stack were fetched by [`EthernetRunner`] before the stack
/// ever asked, which is the whole reason the split exists.
pub type EthernetDriver<'d> = ch::Device<'d, MTU>;

/// The half that talks to the hardware: an endless task moving frames
/// between [`EthernetDriver`]'s queues and the chip's bulk endpoints.
///
/// Created by [`new`] and driven by [`run`](EthernetRunner::run), which an
/// application spawns as its own task.
pub struct EthernetRunner<'d, 'c, E: EthernetAsync> {
...
/// What the interface should be brought up as.
///
/// The runner does the bring-up itself (see [`new`]), so anything the
/// caller used to set on the driver beforehand has to arrive here instead.
///
/// Named for the module rather than called `Config`, which would collide
/// with `embassy_net::Config` in every file that uses both — and every
/// consumer of this adapter uses both.
#[derive(Clone, Copy, Debug)]
pub struct EthernetConfig {
    /// The station MAC address, programmed into the chip and reported to
    /// `embassy-net` so it can answer ARP.
    ///
    /// A Raspberry Pi keeps its in firmware rather than on the chip —
    /// `rpi_hal::mailbox::Mailbox::mac_address` is where it comes from.
    pub mac: [u8; 6],

    /// Whether to pass every multicast frame up to the stack.
    ///
    /// **`false` is the chip's reset state and it is not a neutral
    /// choice.** The receiver comes up taking unicast for its own address
    /// plus broadcast, and dropping multicast before the stack sees it.
    /// Anything speaking only unicast or broadcast never notices — DHCP is
    /// broadcast, so an interface gets an address and looks entirely
    /// healthy — and anything else fails completely rather than partially.
    /// mDNS is the one that catches people: a responder binds its socket,
    /// joins its group, announces, and is never once queried, with nothing
    /// anywhere reporting an error.
    ///
    /// Set it if anything above the stack speaks multicast.
    pub all_multicast: bool,
}

impl EthernetConfig {
    /// The chip's own defaults with `mac` programmed in: broadcast and
    /// matched unicast only. See [`Self::all_multicast`] before keeping
    /// it.
...
/// Wraps an Ethernet controller as an `embassy-net` device, returning the
/// [`EthernetDriver`] to hand to `embassy_net::new` and the
/// [`EthernetRunner`] to spawn.
///
/// **`ethernet` must be enumerated and configured but *not* started.**
/// The runner calls [`EthernetAsync::start_async`] itself, on its first
/// pass, which is why [`EthernetConfig`] exists to carry what the caller would
/// otherwise have set beforehand.
///
/// That is deliberate and worth the awkwardness of handing over an
/// unfinished thing. A driver may configure the chip's answer to an empty
/// bulk IN differently between its blocking and awaited bring-ups —
/// parking a receive needs a NAK, polling one needs the opposite — and
/// starting it with the blocking
/// [`Ethernet::start`](rpi_hal::usb::ethernet::Ethernet::start) and then
/// running it here is neither a compile error nor a failure. It is a
/// receive that never parks, so this runner spins the executor for the
/// life of the interface while every frame still arrives. Owning the
/// bring-up is what makes that unreachable rather than documented.
///
...
pub fn new<'d, 'c, E: EthernetAsync, const N_RX: usize, const N_TX: usize>(
    state: &'d mut EthernetState<N_RX, N_TX>,
    ethernet: E,
    rx_channel: Channel<'c>,
    tx_channel: Channel<'c>,
    timer: &'d Timer,
    config: EthernetConfig,
) -> (EthernetDriver<'d>, EthernetRunner<'d, 'c, E>) {
    // The queues are sized by this module's `MTU`, not the driver's, for
    // the reason that constant gives. Checked rather than assumed: a chip
    // that carried a different frame would otherwise have every oversized
    // frame silently truncated to fit.
    assert!(
        E::MTU == MTU,
        "this adapter's queues are sized for a 1514-byte MTU"
    );
...
impl<E: EthernetAsync> EthernetRunner<'_, '_, E> {
    ...
    pub async fn run(self) -> ! {
        ...
        // Bring the chip up here rather than expecting it started, for the
        // reason `new` gives. Over the receive channel and on endpoint 0:
        // this is control transfers, and happens before either channel
        // takes up its frame duties.
        if let Err(error) = ethernet
            .start_async(&mut rx_channel, timer, config.mac)
            .await
        {
            // Tried once, not retried. A chip that fails here has already
            // answered enumeration and a descriptor read, so this is not a
            // device still waking up — and retrying needs a delay, which
            // needs a time source this module deliberately does not have
            // (see the `embassy-time` note in Cargo.toml: the frame path
            // awaits USB transfers and nothing else).
            //
            // Parking rather than looping, so a dead interface costs
            // nothing instead of spinning the executor forever. The stack
            // sees a link that never comes up, which is the honest outward
            // signal; `start_error` says why.
            START_LAST_ERROR.store(error_code(error), Ordering::Relaxed);
            state_runner.set_link_state(LinkState::Down);
            core::future::pending::<()>().await;
        }

        if config.all_multicast {
            // Reported through the same channel as a bring-up failure but
            // not fatal: an interface that filters multicast still carries
            // everything else, and taking it down entirely would turn a
            // degraded mDNS into no network at all.
            if let Err(error) = ethernet
                .set_all_multicast_async(&mut rx_channel, timer, true)
                .await
            {
                START_LAST_ERROR.store(error_code(error), Ordering::Relaxed);
            }
...
async fn receive_forever<R: EthernetRx>(
...
async fn transmit_forever<T: EthernetTx>(
...
/// [`error_code`] of the failure that stopped bring-up, or [`NO_ERROR`].
static START_LAST_ERROR: AtomicU8 = AtomicU8::new(NO_ERROR);

/// Why the interface failed to start, or `None` if it did not fail.
///
/// `Some` here and a link that never comes up are the same event seen from
/// two places. The runner attempts the bring-up once and then parks, so
/// this does not change afterwards — and unlike the receive and transmit
/// counters, a value here means the interface never carried a frame at
/// all rather than that it dropped some.
///
/// Also set, non-fatally, if the multicast filter could not be configured
/// (see [`EthernetConfig::all_multicast`]). That case leaves the interface working
/// for everything except the traffic that filter would have passed, which
/// is worth being able to tell apart from a total failure — the link state
/// is what distinguishes them.
pub fn start_error() -> Option<TransferError> {
    error_from_code(START_LAST_ERROR.load(Ordering::Relaxed))
}
